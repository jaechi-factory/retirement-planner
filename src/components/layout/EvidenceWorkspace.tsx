        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: cardPadding }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            {inputs.goal.targetMonthly > 0 && (
              ...
              >
                {`월 ${inputs.goal.targetMonthly}만원(${new Date().getFullYear()}년 기준)의 생활 수준을 유지하려면, ${inputs.goal.retirementAge}세(은퇴시점)에는 월 ${Math.round(inputs.goal.targetMonthly * Math.pow(1 + (inputs.goal.inflationRate ?? 2) / 100, inputs.goal.retirementAge - inputs.status.currentAge))}만원이 필요해요. 이후에는 물가상승, 투자수익, 소비가 모두 복리로 반영돼 생활비와 자산이 함께 커져요.`}
              </p>
            )}
          </div>
          <div
